...
describe("verifier / Warburg check (CHK08)", () => {
  ...
  it("CHK08 passes when closed-form residual is within tol", async () => {
    const p = basePayload({ closed_form_residual: 1e-6 });
    const r = await runVerifier(p, computeArtifactHash(p));
    expect(r.issues.some(i => i.check_id === "CHK08")).toBe(false);
    expect(r.verdict).toBe("pass");
  });

  it("CHK08 is silently skipped when closed_form_residual is null", async () => {
    const p = basePayload({
      warburg_nu: null,
      closed_form_residual: null,
    });
    const r = await runVerifier(p, computeArtifactHash(p));
    const warburgIssues = r.issues.filter(i => i.check_id === "CHK08");
  ...
  it("regression: removed CHK09, CHK10, CHK11, and CHK12 are never emitted", async () => {
    const stressPayloads = [
      basePayload({ warburg_nu: 0.5 }),
      basePayload({ warburg_nu: 0.0 }),
      basePayload({ warburg_nu: 1.5 }),
      basePayload({ warburg_nu: -2.0 }),
      basePayload({ warburg_nu: null }),
    ];
    for (const p of stressPayloads) {
      const r = await runVerifier(p, computeArtifactHash(p));
      for (const dead of ["CHK09", "CHK10", "CHK11", "CHK12"]) {
        expect(r.issues.some(i => i.check_id === dead)).toBe(false);
      }
    }
  });
});

describe("warburg-self-test", () => {
  it("startup self-test passes on a healthy build", async () => {
    const { runWarburgSelfTest } = await import("../lib/warburg-self-test.js");
    const r = runWarburgSelfTest();
    expect(r.ok).toBe(true);
    expect(r.phases).toHaveLength(5);
    expect(r.phases.every(p => p.pass)).toBe(true);
    expect(Math.abs(r.mercer_slope + 1)).toBeLessThan(0.4);
  });
});
