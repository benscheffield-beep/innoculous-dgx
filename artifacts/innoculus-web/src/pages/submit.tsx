...
export default function Submit() {
  ...
  const onSubmit = (data: InnoculationFormValues) => {
    const policy_config: CreateJobRequest["policy_config"] = {};
    if (data.spectral_radius_max !== undefined) policy_config.spectral_radius_max = data.spectral_radius_max;
    if (data.cond_limit !== undefined) policy_config.cond_limit = data.cond_limit;
    if (data.dual_error_tol !== undefined) policy_config.dual_error_tol = data.dual_error_tol;
    if (data.spectral_tail_tol !== undefined) policy_config.spectral_tail_tol = data.spectral_tail_tol;
    if (data.warburg_residual_tol !== undefined) policy_config.warburg_residual_tol = data.warburg_residual_tol;
    if (data.judge_disagreement_max !== undefined) policy_config.judge_disagreement_max = data.judge_disagreement_max;
    if (data.min_probes_per_month !== undefined) policy_config.min_probes_per_month = data.min_probes_per_month;
    if (data.min_recheck_count !== undefined) policy_config.min_recheck_count = data.min_recheck_count;
    ...
    const req: CreateJobRequest = {
      ...
      numerical: {
        ...
        precision: {
          b: data.prec_b,
          tol: data.prec_tol,
          ...(data.prec_safety ? { safety_margin: data.prec_safety } : {}),
        },
      },
      cutoff_trace: {
        model: data.model,
        ...
        <CardContent className="p-6">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <Accordion type="multiple" defaultValue={["spectral", "speculative"]} className="space-y-3">
                {/* Spectral phase */}
                <AccordionItem
                  value="spectral"
                  className="border border-white/5 rounded-md bg-black/10 px-4"
                  data-testid="section-spectral"
                >
                  <AccordionTrigger className="hover:no-underline py-3">
                    <div className="flex items-center gap-3">
                      <span className="w-6 h-6 inline-flex items-center justify-center text-base leading-none font-serif text-primary">Θ</span>
                      <span className="font-semibold">Spectral phase</span>
                      <span className="text-xs text-muted-foreground font-mono uppercase tracking-wide">numerical</span>
                    </div>
                  </AccordionTrigger>
                  <AccordionContent className="pb-4 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      {form.watch("kernel_type") === "gaussian" ? (
                        <FormField
                          control={form.control}
                          name="sigma"
                          render={({ field }) => (
                    ...
                    {isDev && (
                      <>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                          <FormField control={form.control} name="trunc_m" render={({ field }) => (
                          ...
                          <FormField control={form.control} name="trunc_r" render={({ field }) => (
                          ...
                          <FormField control={form.control} name="prec_b" render={({ field }) => (
                          ...
                          <FormField control={form.control} name="prec_tol" render={({ field }) => (
                          ...
                          <FormField control={form.control} name="prec_safety" render={({ field }) => (
                        ...
                        <Accordion type="single" collapsible className="w-full border border-white/5 rounded-md bg-black/20">
                          <AccordionItem value="spectral-policy" className="border-b-0">
                            <AccordionTrigger className="px-4 py-3 hover:bg-white/5">Spectral policy thresholds</AccordionTrigger>
                            <AccordionContent className="px-4 pb-4">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                                <FormField control={form.control} name="spectral_radius_max" render={({ field }) => (
                                ...
                                <FormField control={form.control} name="cond_limit" render={({ field }) => (
                                ...
                                <FormField control={form.control} name="dual_error_tol" render={({ field }) => (
                                ...
                                <FormField control={form.control} name="spectral_tail_tol" render={({ field }) => (
                                ...
                                <FormField control={form.control} name="warburg_residual_tol" render={({ field }) => (
                ...
                <AccordionItem
                  value="speculative"
                  className="border border-white/5 rounded-md bg-black/10 px-4"
                  data-testid="section-speculative"
                >
                  <AccordionTrigger className="hover:no-underline py-3">
                    <div className="flex items-center gap-3">
                      <span className="w-6 h-6 inline-flex items-center justify-center text-base leading-none font-serif text-primary">Σ</span>
                      <span className="font-semibold">Speculative phase</span>
                      <span className="text-xs text-muted-foreground font-mono uppercase tracking-wide">cutoff trace</span>
                    </div>
                  </AccordionTrigger>
                  <AccordionContent className="pb-4 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {isDev && (
                      <Accordion type="single" collapsible className="w-full border border-white/5 rounded-md bg-black/20">
                        <AccordionItem value="cutoff-policy" className="border-b-0">
                          <AccordionTrigger className="px-4 py-3 hover:bg-white/5">Speculative policy thresholds</AccordionTrigger>
                          <AccordionContent className="px-4 pb-4">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                              <FormField control={form.control} name="judge_disagreement_max" render={({ field }) => (
                              ...
                              <FormField control={form.control} name="min_probes_per_month" render={({ field }) => (
                              ...
                              <FormField control={form.control} name="min_recheck_count" render={({ field }) => (
