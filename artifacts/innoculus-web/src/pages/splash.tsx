...
/** Spoken-only greeting played when the user first opens the Daemon chat
 *  by clicking the central orb. Vocalised through the browser's Web Speech
 *  API (no pre-baked MP3 since the proxied OpenAI integration does not
 *  expose /audio/speech). It is also seeded into chatHistory so the LLM
 *  has it as conversational context for any follow-up turns the visitor
 *  asks ("what do you mean by that?" etc.) — but it is never rendered as
 *  written text in the sentence bar. */
const DAEMON_GREETING =
...
export default function Splash() {
  ...
  /** Speak the welcome greeting through the browser's Web Speech API.
   *  Called only on the first orb-click of the session (subsequent opens
   *  just play the short tactile daemon cue instead). Mirrors its
   *  speaking state into `speakingFlagRef` so the chat-panel glow lights
   *  up exactly the same way it does for the MP3-backed phrases. */
  const speakGreeting = useCallback(() => {
    if (typeof window === "undefined" || !window.speechSynthesis) return;
    // Cancel any in-flight utterance so we never double-speak.
    window.speechSynthesis.cancel();
    const utter = new SpeechSynthesisUtterance(DAEMON_GREETING);
    utter.rate = 0.92;
    utter.pitch = 0.95;
    utter.volume = 0.95;
    // Prefer an English voice if the browser exposes one. Voice list may
    // be empty on first call in some browsers (loaded async); falling
    // back to the platform default in that case is fine.
    const voices = window.speechSynthesis.getVoices();
    const preferred =
      voices.find(
        (v) =>
          /^en[-_]/i.test(v.lang) &&
          /(samantha|victoria|google|female|natural)/i.test(v.name),
  ...
  const openDaemonChat = useCallback(() => {
    ...
    if (!greetedRef.current) {
      // First open of the session: vocalise the greeting (no written
      // transcript). We still seed chatHistory so the LLM has it as
      // conversational context for any follow-up turns; we just never
      // render it in the sentence bar. We deliberately skip the short
      // "daemon" cue here so it doesn't overlap the spoken greeting.
      greetedRef.current = true;
      setChatHistory([{ role: "assistant", content: DAEMON_GREETING }]);
      speakGreeting();
    } else {
      // Subsequent opens: short tactile daemon cue, no greeting replay.
      speak("daemon");
    }
  }, [chatOpen, speak, speakGreeting]);

  const closeDaemonChat = useCallback(() => {
    // Stop the greeting if the user closes the panel mid-utterance so the
    // voice doesn't keep talking into a closed surface.
